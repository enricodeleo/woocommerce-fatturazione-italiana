 ...
 * - HPOS compatibility by Enrico Deleo (https://enricodeleo.com)
 * - Based on WooCommerce PDF Invoices Italian Add-on by labdav
 *
 */
...
class WC_Piva_Cf_Invoice_Ita {
	...
	public function init() {
		...
		include_once 'includes/class-data-store.php';
	...
	public function woocommerce_order_formatted_billing_address( $fields, $order) {
		if (class_exists('\Automattic\WooCommerce\Utilities\OrderUtil') && \Automattic\WooCommerce\Utilities\OrderUtil::custom_orders_table_usage_is_enabled()) {
			...
			WC_Piva_Cf_Invoice_Ita_Data_Store::instance()->set('invoice_type', $fields['invoice_type']);
	...
	public function localisation_address_format( $formats ) {
		...
		$invoice_type = WC_Piva_Cf_Invoice_Ita_Data_Store::instance()->get('invoice_type');
		if($invoice_type){
			if($invoice_type == 'receipt' || $invoice_type == 'private_invoice'){
				$formats['IT'] .= "\n\n{cf}";
